...
public class ImageUtil {   
    ...
    static public Drawable getBigRoundedUserPhoto(Context context, int idx)
	{
		...
		if (!f.exists())
		{
			...
			photo=ImageUtil.getBitmapAsRoundCorner(userphotoPath,1,7);
		
		return photo;
	}
    
    static public Drawable getUserPhoto(Context context, int idx)
	{
    	Drawable photo=null;
    	String userphotoPath = Global.SdcardPath_inbox + "photo_" + idx + "b.jpg";
		File f = new File(userphotoPath);
		if (!f.exists())
		{
			...
			f = new File(userphotoPath);
			if (f.exists())
				photo=ImageUtil.getBitmapAsRoundCorner(userphotoPath,1,0);
		}else
			photo=ImageUtil.getBitmapAsRoundCorner(userphotoPath,1,0);
