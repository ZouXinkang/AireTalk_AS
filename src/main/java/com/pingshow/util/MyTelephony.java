...
public class MyTelephony {
    ...
    static public String smartAddingPrefix(Context context, String number, boolean [] isMobileNumber)
    {
    	String globalnumber=number;
    	number=cleanPhoneNumber2(number);
		
		init(context);
		
		if (validWithCurrentISO(number))
		{
			globalnumber=MyTelephony.addPrefixWithCurrentISO(number);
			if (isMobileNumber!=null)
				isMobileNumber[0]=true;
		}else{
			if (MyTelephony.validLandLineWithCurrentISO(number))
			{	
				if (isMobileNumber!=null)
					isMobileNumber[0]=false;
				globalnumber=MyTelephony.addPrefixLandLineWithCurrentISO(number);
			}
		}
		
		if (!globalnumber.startsWith("+"))
		{
			globalnumber=MyTelephony.attachPrefix(context, number);
			if (globalnumber.startsWith("+")) 
			{
				if (isMobileNumber!=null)
					isMobileNumber[0]=true;
		...
		if (!globalnumber.startsWith("+"))
		{
			globalnumber=MyTelephony.attachFixedPrefix(context, number);
			if (globalnumber.startsWith("+"))
			{
				if (isMobileNumber!=null)
					isMobileNumber[0]=false;
