...
import com.pingshow.video.display.OpenGLESDisplay;

public class GLVideoWindow {
	...
	public static interface VideoWindowListener{
		void onVideoRenderingSurfaceReady(GLVideoWindow vw, SurfaceView surface);
		void onVideoRenderingSurfaceDestroyed(GLVideoWindow vw);
	};
	 ...
	 */
	public GLVideoWindow(SurfaceView renderingSurface) {
	...
	public void init(int index) {
		...
		mVideoRenderingView.getHolder().addCallback(new Callback(){
			public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
				Log.i( "GL Video display surface is being changed.");
				if (!useGLrendering) {
					synchronized(GLVideoWindow.this){
						mBitmap=Bitmap.createBitmap(width,height,Config.RGB_565);
						mSurface=holder.getSurface();
					}
				}
				if (mListener!=null) mListener.onVideoRenderingSurfaceReady(GLVideoWindow.this, mVideoRenderingView);
				Log.w( "GL Video display surface changed");
			}

			public void surfaceCreated(SurfaceHolder holder) {
				Log.w( "GL Video display surface created");
			}

			public void surfaceDestroyed(SurfaceHolder holder) {
				Log.d( "GL Video display surface destroyed is called"); 
				if (!useGLrendering) {
					synchronized(GLVideoWindow.this){
						mSurface=null;
						mBitmap=null;
				...
				if (mListener!=null)
					mListener.onVideoRenderingSurfaceDestroyed(GLVideoWindow.this);
				Log.d( "GL Video display surface destroyed"); 
			}
		});
	...
	public Surface getSurface(){
		if (useGLrendering)
			Log.e( "GL View class does not match Video display filter used (you must use a non-GL View)");
	...
	public void setOpenGLESDisplay(int ptr) {
		if (!useGLrendering)
			Log.e( "GL View class does not match Video display filter used (you must use a GL View)");
		if (renderer!=null) renderer.setOpenGLESDisplay(ptr);
	}
    ...
    private static class Renderer implements GLSurfaceView.Renderer {
    	...
    	public void setOpenGLESDisplay(int ptr) {
    		 ...
    		 * - they are called from different threads (Rendering thread and VoipCore's one)
