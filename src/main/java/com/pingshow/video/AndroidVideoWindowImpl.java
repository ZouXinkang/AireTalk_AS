...
public class AndroidVideoWindowImpl {
	...
	public static SurfaceView mVideoPreviewView;
	...
	public void init() {
		...
		mVideoRenderingView.getHolder().addCallback(new Callback(){
			public void surfaceChanged(SurfaceHolder holder, int format,
				...
				Log.i( "AD Video display surface is being changed.");
				if (!useGLrendering) {
				...
				Log.w( "AD Video display surface changed");
			}

			public void surfaceCreated(SurfaceHolder holder) {
				Log.w( "AD Video display surface created");
			}

			public void surfaceDestroyed(SurfaceHolder holder) {
				Log.d( "AD Video display surface destroyed is called"); 
				if (!useGLrendering) {
					synchronized(AndroidVideoWindowImpl.this){
						mSurface=null;
						mBitmap=null;
				...
				Log.d( "AD Video display surface destroyed"); 
			}
		});
		...
		mVideoPreviewView.getHolder().addCallback(new Callback(){
			public void surfaceChanged(SurfaceHolder holder, int format,
					int width, int height) {
				Log.i( "AD Video preview surface is being changed.");
				if (mListener!=null) 
					mListener.onVideoPreviewSurfaceReady(AndroidVideoWindowImpl.this, mVideoPreviewView);
				Log.w( "AD Video preview surface changed");
			}

			public void surfaceCreated(SurfaceHolder holder) {
				Log.w( "AD Video preview surface created");
			}

			public void surfaceDestroyed(SurfaceHolder holder) {
				if (mListener!=null)
					mListener.onVideoPreviewSurfaceDestroyed(AndroidVideoWindowImpl.this);
				Log.d( "AD Video preview surface destroyed");
			}
		});
	...
	public Surface getSurface(){
		if (useGLrendering)
			Log.e( "AD View class does not match Video display filter used (you must use a non-GL View)");
	...
	public void setOpenGLESDisplay(int ptr) {
		if (!useGLrendering)
			Log.e( "AD View class does not match Video display filter used (you must use a GL View)");
