...
public class RegisterActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    ((Button) findViewById(R.id.register)).setOnClickListener(new OnClickListener() {
    		public void onClick(View v) {
    			...
    			if (devIsPhone && withSIM && !check_phonenumber(xusername)) {
    				// check phone number 
    				Message msg = new Message();
    				msg.obj = "fail,phonenumber";
    				mHandler.sendMessage(msg);
    				return;
    			}else if (!devIsPhone && !check_username(xusername)) {
    				// check user name 
    				Message msg = new Message();
				...
				if(!phoneNumberReadable && withSIM && MyTelephony.isPhoneNumber(xusername)){
		...
		intentToReceiveFilter.addAction("com.pingshow.airecenter.registration"); 
	...
	Runnable popupFakeRegisterSms=new Runnable()
	{
		...
		public void run() {
		    ...
		    it.setAction("com.pingshow.airecenter.registration");
...
//		return email.matches("^[0-9a-zA-Z_\\-\\.]+@[0-9a-zA-Z_\\.]+$");
	...
	Runnable mRegisterTask = new Runnable() {
        public void run() {
        	...
        	if (devIsPhone && withSIM && !check_phonenumber(username)) {
				// check phone number 
				Message msg = new Message();
				msg.obj = "fail,phonenumber";
				mHandler.sendMessage(msg);
    ...
    Runnable mStraightRegisterTask = new Runnable() {
        public void run() {
			...
			try{
				...
				String Md5=MyUtil.getMD5(src.getBytes());
				int count=0;
				String Return="";
				do{
    ...
    BroadcastReceiver waitingForRegistrationSMS = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent){
			if (intent.getAction().equals("com.pingshow.airecenter.registration") && SmsConfirmWaiting)
			{
				SmsConfirmWaiting=false;
				mHandler.post(dismissProgressDialog);
		...
		else if (requestCode==64)
		{
			if (resultCode==RESULT_OK) {
				...
				if(!phoneNumberReadable && MyTelephony.isPhoneNumber(xusername)){
					...
					Cursor cursor1 = null;
					try {
						String md5=MyUtil.getMD5(xusername.getBytes());
