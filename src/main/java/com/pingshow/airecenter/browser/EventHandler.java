			...
			case R.id.back_button:			
				if (mFileMang.getCurrentDir() != "/") {
					if(multi_select_flag) {
						...
						Toast.makeText(mContext, mContext.getResources().getString(R.string.multisel_off), 
									   Toast.LENGTH_SHORT).show();
					}
					
					stopThumbnailThread();
					if ((mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_record)
							|| (mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_video)
							|| (mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_image)
							|| (mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_files)
							|| (mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_music)) {
						updateDirectory(mFileMang.setHomeDir(trimDir(Global.SdcardPath_airetalk)));
					} else if ((mFileMang.getCurrentDir() + "/").equals(Global.Storage_USBSD)) {
						updateDirectory(mFileMang.setHomeDir("/storage"));
					} else if ((mFileMang.getCurrentDir() + "/").equals(Global.SdcardPath_airetalk)) {
...
//					mPathLabel.setText(mFileMang.getCurrentDir());
			...
			case R.id.ext_button:
				...
				stopThumbnailThread();
				updateDirectory(mFileMang.setHomeDir(trimDir(Global.Storage_USBSD)));
				if(mPathLabel != null)
					mPathLabel.setText(mFileMang.getCurrentDir());
			...
			case R.id.hidden_attach:
    			...
    			mContext.startActivity(Intent.createChooser(mail_int, 
    					mContext.getResources().getString(R.string.bwr_email) + "..."));
			...
			case R.id.hidden_copy:
				...
				mInfoLabel.setText(mContext.getResources().getString(R.string.bwr_holding) + " " + mMultiSelectData.size() + 
								   " " + mContext.getResources().getString(R.string.bwr_files));
				
				mDelegate.killMultiSelect(false);
				MainBrowser._this.showPasteV(true);
				MainBrowser._this.updateStorageLabel();
				break;
			...
			case R.id.hidden_delete:
				...
				builder.setMessage(mContext.getResources().getString(R.string.del_confirm1) + " (" +
								    data.length + ") " + mContext.getResources().getString(R.string.del_confirm2));
				builder.setCancelable(false);
				builder.setPositiveButton(mContext.getResources().getString(R.string.bwr_delete), new DialogInterface.OnClickListener() {
					@Override
				...
				builder.setNegativeButton(mContext.getResources().getString(R.string.bwr_cancel), new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						mDelegate.killMultiSelect(true);
						dialog.cancel();
    ...
    public class TableRow extends ArrayAdapter<String> {
    	...
    	public TableRow() {
    		super(mContext, R.layout.bwr_tablerow, mDataSource);    		
    	}
    	...
    	public void killMultiSelect(boolean clearData) {
    		...
    		if(clearData) {
				MainBrowser._this.showPasteV(false);
    			if(mMultiSelectData != null && !mMultiSelectData.isEmpty()) {
    				mInfoLabel.setText("");
    	...
    	public View getView(int position, View convertView, ViewGroup parent) {
    		...
    		if(convertView == null) {
    			...
    			convertView = inflater.inflate(R.layout.bwr_tablerow, parent, false);
    		...
    		if(file != null && file.isFile()) {
    			...
    			if (sub_ext.equalsIgnoreCase("pdf")) {
    				...
    				if(thumbnail_flag && file.length() != 0) {
    					...
    					mViewHolder.icon.setImageResource(R.drawable.bwr_image);
    				}
    				...
    				mViewHolder.icon.setImageResource(R.drawable.bwr_zip);
    			
    			} else if(sub_ext.equalsIgnoreCase("m4v") ||
    					  sub_ext.equalsIgnoreCase("wmv") ||
    					  sub_ext.equalsIgnoreCase("3gp") || 
    					  sub_ext.equalsIgnoreCase("ogg") || 
    					  sub_ext.equalsIgnoreCase("rmvb") || 
    					  sub_ext.equalsIgnoreCase("avi") || 
    					  sub_ext.equalsIgnoreCase("mp4")) {
    				
    				mViewHolder.icon.setImageResource(R.drawable.bwr_movies);
    			
    			} else if(sub_ext.equalsIgnoreCase("doc") || 
    					  sub_ext.equalsIgnoreCase("docx")) {
    				
    				mViewHolder.icon.setImageResource(R.drawable.bwr_word);
    			
    			} else if(sub_ext.equalsIgnoreCase("xls") || 
    					  sub_ext.equalsIgnoreCase("xlsx")) {
    				
    				mViewHolder.icon.setImageResource(R.drawable.bwr_excel);
    				
    			} else if(sub_ext.equalsIgnoreCase("ppt") ||
    					  sub_ext.equalsIgnoreCase("pptx")) {
    				
    				mViewHolder.icon.setImageResource(R.drawable.bwr_ppt);   	
    				
    			} else if(sub_ext.equalsIgnoreCase("html")) {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_html32);  
    				
    			} else if(sub_ext.equalsIgnoreCase("xml")) {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_xml32);
    				
    			} else if(sub_ext.equalsIgnoreCase("conf")) {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_config32);
    				
    			} else if(sub_ext.equalsIgnoreCase("apk")) {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_appicon);
    				
    			} else if(sub_ext.equalsIgnoreCase("jar")) {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_jar32);
    				
    			} else {
    				mViewHolder.icon.setImageResource(R.drawable.bwr_text);
    			}
    			
    		} else if (file != null && file.isDirectory()) {
    			if (file.canRead() && file.list().length > 0)
    				mViewHolder.icon.setImageResource(R.drawable.bwr_folder_full);
    			else
    				mViewHolder.icon.setImageResource(R.drawable.bwr_folder);
    		}
    ...
    private class BackgroundWork extends AsyncTask<String, Void, ArrayList<String>> {
    	...
    	protected void onPreExecute() {
    		...
    		switch(type) {
    			case SEARCH_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, mContext.getResources().getString(R.string.bwr_searching1), 
    						mContext.getResources().getString(R.string.bwr_searching1) + "...",
    												true, true);
    				break;
    				
    			case COPY_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, mContext.getResources().getString(R.string.bwr_copying1), 
    						mContext.getResources().getString(R.string.bwr_copying1) + "...", 
    												true, false);
    				break;
    				
    			case UNZIP_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, mContext.getResources().getString(R.string.bwr_unzipping1), 
    						mContext.getResources().getString(R.string.bwr_unzipping1) + "...",
    												true, false);
    				break;
    			...
    			case ZIP_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, mContext.getResources().getString(R.string.bwr_zipping1), 
    						mContext.getResources().getString(R.string.bwr_zipping1) + "...", 
    												true, false);
    				break;
    				
    			case DELETE_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, mContext.getResources().getString(R.string.bwr_deleting1), 
    						mContext.getResources().getString(R.string.bwr_deleting1) + "...", 
    												true, false);
    				break;
    		}
    	}
		...
		protected void onPostExecute(final ArrayList<String> file) {			
			...
			switch(type) {
				case SEARCH_TYPE:				
					if(len == 0) {
						Toast.makeText(mContext, mContext.getResources().getString(R.string.bwr_notfound) + " " + file_name, 
											Toast.LENGTH_SHORT).show();
				...
				case COPY_TYPE:
					...
					if(copy_rtn == 0)
						Toast.makeText(mContext, mContext.getResources().getString(R.string.bwr_copypasteok), 
											Toast.LENGTH_SHORT).show();
					else
						Toast.makeText(mContext, mContext.getResources().getString(R.string.bwr_copypastefail), Toast.LENGTH_SHORT).show();
