...
//		String phpIP = AireJupiter.myLocalPhpServer;
			...
			try
    		{
				...
				do{
					...
					if(Return.startsWith("Done"))
						break;
					count++;
					Log.e("tml upload upload upload upload");
					MyUtil.Sleep(2500);
				}while(count<4);
    		}catch(Exception e){
    			Log.e("sfa1 " + e.getMessage());
    		}
			if (!Return.contains("Done")) {
				Log.i("tml UploadMessageToServer ***BAD*** Return=" + Return);
			}
    		ConversationActivity.fileUploading = false;
    		if (Return.startsWith("Done"))
	        {
    			Log.e("tml UploadMessageToServer DONE");
    			uploadSucess = 1;
		...
		if (!multiple)
		{
			...
			for (int i=0;i<sendeeAddressList.size();i++)  //TODO multithread
			{
				try{
