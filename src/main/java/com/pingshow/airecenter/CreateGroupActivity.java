...
public class CreateGroupActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		((Button)findViewById(R.id.pickup)).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent it=new Intent(CreateGroupActivity.this, PickupActivity.class);
		...
		((Button)findViewById(R.id.done)).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				groupName=((EditText)findViewById(R.id.nickname)).getText().toString();
	    		if (groupName!=null)
	    		{
	    			groupName=groupName.trim();
	        		boolean chinese=groupName.toLowerCase().equals(groupName.toUpperCase());
	        		
	        		if (groupName.length()<(chinese?3:6) || groupName.length()>30)
	        		{
	    		...
	    		progress = ProgressDialog.show(CreateGroupActivity.this, "", getString(R.string.in_progress), true, true);
		...
		public void run()
		{
			...
			try {
				...
				do {
					...
					Return = net.doPostHttps("create_group.php", "id=" + myIdx
    						+"&members=" + members
    						+"&name=" + URLEncoder.encode(groupName, "UTF-8"), null);
			...
			if (photoAssigned)
			{
				...
				String localPath=Global.SdcardPath_inbox +"photo_"+(groupidx+100000000)+".jpg";
				...
				try {
					...
					do {
						MyNet net = new MyNet(CreateGroupActivity.this);
						Return = net.doPostAttach("uploadgroupphoto.php", groupidx, 0, localPath, null); // httppost
						if (Return.startsWith("Done"))
							break;
						MyUtil.Sleep(2500);
					} while (++count < 3);
			...
			SendAgent agent=new SendAgent(CreateGroupActivity.this, myIdx, 0, true);
	...
	private void arrangePickedUsers()
	{
		...
		try{
			...
			if (path == null) {
				...
				if (MyUtil.checkSDCard(getApplicationContext()) && (new File(path).exists()))
			...
			if (path == null || !MyUtil.checkSDCard(getApplicationContext()) || !(new File(path).exists()))
			{	
			}else{
				sendeeList.add("0");
				showMyself=true;
			...
			for(int i=0;i<count;i++)
			{
				...
				if (showMyself && i==count-1) userphotoPath=path;
				...
				RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams((int)(mDensity*w), (int)(mDensity*w));
		...
		if (resultCode == RESULT_OK) {
			if (requestCode == 108) {
		    	...
		    	try { 
		        	...
		        	ResizeImage.ResizeXY(this, SrcImagePath, outFilename, 240, 100);
					...
					{
						Intent it = new Intent(CreateGroupActivity.this, PictureRotationActivity.class);
