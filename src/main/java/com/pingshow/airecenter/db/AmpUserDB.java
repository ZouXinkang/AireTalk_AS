...
public class AmpUserDB {
	...
	private static final String FFY_DB_CREATE =
		...
		KEY_EMAIL		+ " TEXT NULL, " +
		KEY_PRIVILEGE	+ " INTEGER DEFAULT 0); ";
	...
	{
		return insertUser(address, idx, nickname, null, null, null, null, 0);
	}
	
	public synchronized long insertUser(String address,int idx, String nickname, String title, 
			String phone, String mobile, String email, int privilege) 
	{
		if(!mDb.isOpen()) return -1;
		if (address.length()<6) return -1;
		if (isFafauser(address) || isFafauser(idx)) return -1;
    	...
    	if (cursor.getCount()>0)
    	{
        	...
        	vals.put(KEY_MOBILE, mobile);
        	vals.put(KEY_EMAIL, email);
        	vals.put(KEY_PRIVILEGE, privilege);
	...
	public String getInfoByIdx(int idx, String key)
	{
		if(!mDb.isOpen()) return "";
		String info="";
		Cursor cursor =mDb.query(true, AMP_USER_DB_TABLE,
				new String [] {key}, KEY_IDX + " = " + idx, null,
				null, null, null, null);
		if(cursor.moveToFirst())
			info = cursor.getString(0);
		cursor.close();
		Log.w("addF.mADB getInfoByIdx=" + idx + " " + key);
		return info;
	}
}
