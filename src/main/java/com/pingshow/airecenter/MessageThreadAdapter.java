...
public class MessageThreadAdapter extends CursorAdapter {
	...
	public void bindView(final View view, final Context context, final Cursor c) {
		...
		if(s!= null && s.startsWith(context.getString(R.string.filememo_send))&&s.contains("(fl)")){
			s="(fl)";
		}
			...
			for(int i=0;i<Smiley.MAXSIZE;i++)
			{
				...
				{ 
					if(i==(Smiley.MAXSIZE-1)){//picture
						Drawable d = ImageUtil.loadBitmapSafe(c.getString(7),20);
						if(d==null){
							...
							//ImageSpan icon = new ImageSpan(mContext, bitmap, ImageSpan.ALIGN_BOTTOM);
