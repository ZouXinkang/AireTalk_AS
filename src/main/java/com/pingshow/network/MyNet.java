...
import com.pingshow.airecenter.contacts.XMLContentHandler;
...
//			HostURL = "http://" + AireJupiter.myPhpServer + "/onair/";
	...
	public String doPostHttps(String surl, String data, String phpIP) {
		...
		try {
			...
			Log.e("doPostHttps ERR: " + e.getMessage());
		}
	...
	public List<RelatedUserInfo> doPostHttpsWithXML(String surl, String data, String phpIP) {
		...
		try {
			...
			Log.e("doPostHttpsXML ERR: " + e.getMessage());
		}
	...
	public List<RelatedUserInfo> doPostHttpWithXML(String surl, String data, String phpIP) {
		...
		try {
			...
			Log.e("doPostHttpXML ERR: " + e.getMessage());
		}
	...
	{
		Log.i("net.stopLoading " + filename);
		try{
			HttpUriRequest post=(HttpUriRequest)mHttpPosts.get(filename);
			if (post!=null) {
				post.abort();
				Log.e("**ABORT! doPostAttach " + filename);
			}
		}catch(Exception e){
			Log.e("stopUploading ERR " + e.getMessage());
		}
	}
	...
	public String doPostAttach8(String surl,int myidx,String fn, String filename,String phpIP){
		...
		try {
			...
			Log.e("doPostAttach8 ERR: "+e.getMessage());
		}
		Log.i("net.postAttach8! " + sURL + " :: " + Return);
		return Return;
	}

	public String doPostAttachSUV(String surl,int myidx,String fn, String filename){
		String Return="";
    	String sURL = "";
    	sURL = surl;

		File file = new File(filename);
    	Log.i("net." + sURL + " << " + myidx + " " + fn + " " + filename + " file?" + file.exists());
		try {
			HttpPost httpPost = new HttpPost(sURL);
			mHttpPosts.put(filename, httpPost);
			
			MyPreference mPref = new MyPreference(mContext);
			String myname = mPref.read("myPhoneNumber");
			FileBody fileBody = new FileBody(file);
			StringBody myid = new StringBody(myname+""); 
			StringBody id = new StringBody(myidx+""); 
			StringBody sFn = new StringBody(fn);
			MyMultipartEntity reqEntity = new MyMultipartEntity(file.length(), filename);
			reqEntity.addPart("id", myid);
			reqEntity.addPart("idx", id);
			reqEntity.addPart("fn", sFn);
			reqEntity.addPart("ufile", fileBody);
		...
		Log.i("net.postAttachSUV! " + sURL + " :: " + Return);
		return Return;
	}
...
//	    	HttpURLConnection URLConn = (HttpURLConnection) url.openConnection();
	    	...
	    	Log.e("Download Failed1 ERR: "+e.getMessage());
...
//	    	HttpURLConnection URLConn = (HttpURLConnection) url.openConnection();
	    	...
	    	Log.e("anyDownload Failed3 ERR: "+e.getMessage());
	    }
...
//	    	HttpURLConnection URLConn = (HttpURLConnection) url.openConnection(); 
	...
	public String doAnyPostHttps(String fullUrl, String data) {
		...
		try {
			...
			Log.e("doAnyPostHttps ERR: " + e.getMessage());
		}
		Log.i("net.anyPostHttps! " + fullUrl + " :: " + Return);
		return Return;
	}
	
	public String doAnyPostHttp(String ip) throws IOException
	{
		return doAnyPostHttp(ip, null);
	}
	...
	public String doAnyPost(String sURL, String data) 
	{
		NetType netType = NetworkControl.getNetType(mContext);
		if (!getNetStatus()) return "";
    	Log.i("net." + sURL + " << " + data);
