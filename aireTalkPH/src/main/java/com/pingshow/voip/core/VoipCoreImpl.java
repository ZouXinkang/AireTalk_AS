...
class VoipCoreImpl implements VoipCore {
	...
	private long d = 0;
	private native long newVoipCore(VoipCoreListener listener, String object, String mode, int sdkVersion, int monotor,int hardwareSupportedHD,int sipTransportType);
	...
	private native boolean setVideoZoom(long d, int x, int y, int ratio, int reserved);  //vivid*** zoom
	private native boolean zoomVideo(long d, int ratio, int x, int y);
	private native int [] getVideoRemoteResolution(long d);
	
	VoipCoreImpl(VoipCoreListener listener, String stun, int audio_port, int video_port, int mode, int monitor,int hardwareSupportedHD,int sipTransportType) throws IOException {
		mListener=listener;
		...
		d = newVoipCore(listener, stun, Mode, Version.sdk(), monitor,hardwareSupportedHD,sipTransportType);
	}
