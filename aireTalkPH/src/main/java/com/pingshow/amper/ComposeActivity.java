...
public class ComposeActivity extends Activity implements OnClickListener{
	...
	protected void onDestroy() {
		try {
			...
			vmp = null;
			myVP1 = null;
			Log.e("Compose onDestroy !@#$ " + e.getMessage());
		}
...
//		MobclickAgent.onPause(this);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 108) {
			...
			if (resultCode == RESULT_OK) {
				mAttached = 0;
				long lon = mPrf.readLong("longitude", 116349386);
				long lat = mPrf.readLong("latitude", 39976279);
			...
			if (requestCode == 1 || requestCode == 3) {
				...
				if (SrcImagePath==null)
				{
					...
					if (ni.netType >= NetInfo.MOBILE_3G)
						...
						result = ResizeImage.Resize(this, SrcImagePath, filename,
								480, 480, 80);  //tml*** bitmap quality, 360->480, 60>80
					if (result == -1) {
	...
	private void addMsgToDatabase(boolean isFile) {
		...
		for (int i=0;i<addressList.size();i++)
		{
			...
			msg.longitudeE6 = mPrf.readLong("longitude", 116349386);
			msg.latitudeE6 = mPrf.readLong("latitude", 39976279);
	...
	public void stopPlayingVoice() {
		try {
			...
			Log.e("Compose stopplayingvoice !@#$ " + e.getMessage());
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.attachment:
			...
			break;
		case R.id.voicesms:
			if(!MyUtil.checkSDCard(ComposeActivity.this)){
				Toast.makeText(this, getString(R.string.no_sdcard),
						Toast.LENGTH_SHORT).show();
				return;
			}
...
//			SrcAudioPath = Global.SdcardPath_sent + getRandomName() + ".mp3";  //tml*** new vmsg
		...
		case R.id.voice:
			if (mp2 != null && mp2.isPlaying())
				return;
			if (myVP2 != null && myVP2.isPlaying())  //tml*** new vmsg
				return;
			if (vmp != null)
				return;
			if (myVP1 != null)
			...
			if (SrcAudioPath != null &&  (new File(SrcAudioPath).length()>0)) {
				try {
					...
					Log.e("Compose voice1 !@#$ " + e.getMessage());
				} catch (IllegalArgumentException e) {
					Log.e("Compose voice2 !@#$ " + e.getMessage());
				} catch (IllegalStateException e) {
					Log.e("Compose voice3 !@#$ " + e.getMessage());
				}
			}
			break;
