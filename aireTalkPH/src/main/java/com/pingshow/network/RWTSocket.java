...
import com.pingshow.amper.PublicWalkieTalkie;
...
public class RWTSocket {
	...
	static public String ServerIP = "144.76.2.81";
	...
	public synchronized boolean Login(int versionCode) {
		...
		while(count++<2 && !connected)
		{
			...
			ServerIP=mPrf.read("RWTServerIP","5.9.180.106");
			...
			try {
				...
				Log.e("rwt Socket Create !@#$" + e.getMessage());
				Logging = 0;
				alter++;
			}
		}
		...
		try {
			...
			Log.e("rwt Login Failed !@#$ " + e.getMessage());
			disconnect();
			Logging = 0;
			return false;
		}
		...
		{
			Log.e("rwt Login, Failed : 999/" + fromServer.substring(3));
			//disconnect();
			Logging = 0;
			return false;
		}
	...
	public void sendRaw(int receiver, byte [] payload, int seq, int length) {
		...
		if (receiver==0 && this.chflag==0) return;
	...
	public boolean channelSwitch(int chFlag) {
		...
		try {
			...
			Log.e("rwt FailtoSendtoServer.400");
			disconnect();
		}
	...
	public boolean channelSwitch(int ch, String iso) {
		...
		this.channel=ch;
		this.iso=iso;
		this.chflag=(iso.charAt(0)<<24)|(iso.charAt(1)<<16)|ch;
		try {
			thrClient.reset400();
			String buffer = "400/" + myId + "/" + Integer.toHexString(this.chflag);			
			Log.i(buffer);
			outToServer.flush();//alec
			outToServer.write(MyUtil.encryptTCPCmd(buffer));
		} catch (Exception e) {
	...
	public boolean leaveChannel() {
		...
		try {
			...
			Log.e("rwt FailtoSendtoServer.420");
			return false;
		}
	...
	public synchronized boolean queryFriendsOnlineStatus() {
		if (logged != 1 || Logging==1 || outToServer==null || thrClient==null) return false;
		long now = new Date().getTime();
		long last = mPrf.readLong("last_rwt_query_status", 0);
		if (now - last < 3000) // 3 sec
